...
const apiKey = db.db.prepare('SELECT api_key FROM businesses WHERE id = ?').get(slug).api_key;
const password = 'admin-' + crypto.randomBytes(4).toString('hex');
db.createAdmin(business.id, adminUser, password);
