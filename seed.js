...
if (db.getBusinessById(BUSINESS_ID)) {
...
for (const [q, a, k] of FAQS) db.addFaq(business.id, q, a, k);
...
for (const [n, status, eta, carrier, tracking, items] of ORDERS) {
  db.upsertOrder(business.id, { order_number: n, status, eta, carrier, tracking_number: tracking, items });
}

const adminUser = 'glowadmin';
const adminPass = 'glow-' + crypto.randomBytes(3).toString('hex');
db.createAdmin(business.id, adminUser, adminPass);
