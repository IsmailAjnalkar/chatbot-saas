 ...
 * Returns { ok, error? }.
 */
async function refundShopifyOrder(shopDomain, accessToken, order, amount) {
  ...
  // Partial when a positive amount below the order total was requested; anything
  // at/above the total (or an invalid amount) falls back to a full refund.
  const isPartial = Number.isFinite(amt) && amt > 0 && Number.isFinite(total) && amt < total - 0.005;
  const refundAmount = isPartial ? amt.toFixed(2) : null;
  const saleTxns = transactions.filter((t) => t.kind === 'sale' && t.status === 'success');
  if (!saleTxns.length) return { ok: false, error: 'no successful payment transaction found on this order' };
  // Partial refunds: money-only, no restock, whole amount on the first sale
  // transaction (assigning it to every transaction would over-refund).
  // Full refunds: restock line items and refund each transaction in full.
  const refundTransactions = isPartial
    ? [{ parent_id: saleTxns[0].id, amount: refundAmount, kind: 'refund', gateway: saleTxns[0].gateway }]
    : saleTxns.map((t) => ({ parent_id: t.id, amount: t.amount, kind: 'refund', gateway: t.gateway }));
  const lineItems = isPartial ? [] : (order.line_items || []).map((li) => ({
    line_item_id: li.id,
  ...
  const body = {
    refund: {
      ...
      notify: true,
      note: isPartial ? `Partial refund of ${order.currency} ${refundAmount} issued via chatbot`
                      : 'Refund issued via chatbot',
  ...
  return { ok: true, partial: isPartial, amount: isPartial ? refundAmount : total.toFixed(2) };
}
