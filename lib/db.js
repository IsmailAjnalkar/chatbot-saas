...
/**
 * Find a business by its contact or support email (case-insensitive). Used
 * for duplicate-account protection at signup and for the public
...
async function findBusinessByContactEmail(email) {
  ...
  const cond = USE_PG
    ? "(LOWER(settings::json ->> 'contact_email') = ? OR LOWER(settings::json ->> 'support_email') = ?)"
    : "(LOWER(json_extract(settings, '$.contact_email')) = ? OR LOWER(json_extract(settings, '$.support_email')) = ?)";
  const row = await one(`SELECT * FROM businesses WHERE ${cond} LIMIT 1`, [normalized, normalized]);
  return row ? decorateBusiness(row) : null;
}
...
module.exports = {
  ...
  findBusinessByContactEmail: withReady(findBusinessByContactEmail),
