...
const AMOUNT_RE = /\$\s?(\d+(?:\.\d{1,2})?)|\b(\d+(?:\.\d{1,2})?)\s?(dollars|usd)\b/i;

/**
 * Flag a session as needing a human, with a context summary the agent sees
 * on takeover. Centralizes the three escalation paths so the inbox always
 * gets the same handoff context.
 */
async function flagForHuman(sessionId, reason, lastVisitorMessage, businessId) {
  const r = String(reason || 'visitor asked for a human').slice(0, 200);
  const v = String(lastVisitorMessage || '').slice(0, 300);
  const summary = v
    ? `Reason: ${r}\nVisitor's last message: "${v}"`
    : `Reason: ${r}`;
  await db.updateSession(sessionId, { flagged_human: 1, unread_admin: 1, handoff_summary: summary });
  if (businessId) {
    // Lightweight ticketing: every escalation opens a ticket (deduped per session)
    // so nothing falls through the cracks. Priority follows session sentiment.
    try {
      const existing = await db.findOpenTicketForSession(businessId, sessionId);
      if (!existing) {
        const sess = await db.getSession(sessionId);
        const priority = sentiment.priorityForSentiment(sess ? (sess.sentiment || 0) : 0);
        const ticket = await db.createTicket(businessId, { session_id: sessionId, subject: r, priority });
        webhooks.emit(businessId, 'ticket.created', { ticket });
      }
    } catch (e) { console.warn('[bot] ticket create failed:', e.message); }
    webhooks.emit(businessId, 'handoff.requested', { session_id: sessionId, reason: r });
  }
}

/**
 * Deterministic agentic intent detection (cancel/refund), so those actions
 * work with or without an LLM configured. Returns { name, orderNumber, amount }
 * or null. Cancel/refund only count when an order is referenced.
 */
function actionIntent(message) {
  const text = String(message || '');
  const isCancel = /\bcancel/i.test(text);
  const isRefund = /\brefund/i.test(text);
  if (!isCancel && !isRefund) return null;
  if (!ORDER_RE.test(text) && !/\border\b/i.test(text)) return null;
  const m = text.match(ORDER_RE);
  const orderNumber = m ? normalizeOrderNumber(m[0]) : '';
  let amount = null;
  if (isRefund) {
    const am = text.match(AMOUNT_RE);
    amount = am ? parseFloat(am[1] || am[2]) : NaN;
  }
  return { name: isCancel ? 'cancel_order' : 'issue_refund', orderNumber, amount };
}

/** Confirmation prompt for a mutating agentic action. */
function confirmPrompt(name, args) {
  if (name === 'cancel_order') {
    return `I can cancel order ${args.order_number} for you. Reply "Yes, cancel it" to confirm, or "no" to stop.`;
  }
  const amt = Number(args.amount);
  return `I can issue a refund of $${amt.toFixed(2)} for order ${args.order_number}. Reply "Yes, refund it" to confirm, or "no" to stop.`;
}
...
// Formal autonomy levels (settings.autonomy):
// - supervised: mutating actions never execute — they route to a human for approval.
// - standard:   confirmations + refund_auto_approve_limit (default).
// - autonomous: confirmations skipped; refund auto-approve limit doubled.
function autonomyLevel(business) {
  const lvl = String(getSetting(business, 'autonomy', 'standard') || 'standard').toLowerCase();
  return ['supervised', 'standard', 'autonomous'].includes(lvl) ? lvl : 'standard';
}
...
async function detectLanguage(business, session, message) {
  ...
  if (cfg.enabled && message && message.trim().length >= 3) {
    try {
      const out = await llm.chatCompleteMetered(business, cfg, [
 ...
 */
async function processMessage({ business, apiKey, sessionId, text, visitorLabel = '', isTest = false }) {
  // Test mode (QA playground): no webhook fan-out, no ticket creation; session marked is_test.
  const emit = (event, data) => { if (!isTest) webhooks.emit(business.id, event, data); };
  ...
  if (isTest && !session.is_test) await db.updateSession(sessionId, { is_test: 1 });
  const message = (text || '').trim();

  // Free-trial enforcement: an expired trial bot answers with an upgrade
  // notice instead of serving customers.
  const bSettings = business.settings || {};
  if (bSettings.plan === 'trial' && bSettings.trial_ends_at && Date.now() > bSettings.trial_ends_at) {
    const messageId = await db.addMessage(sessionId, 'assistant',
      "This chatbot's free trial has ended, so I'm paused for now. If you're the store owner, log in to your dashboard to pick a plan and keep me running — it takes a minute.",
      { type: 'trial_expired', confidence: 1, business: business.name }, 'text');
    return { sessionId, reply: "This chatbot's free trial has ended, so I'm paused for now. If you're the store owner, log in to your dashboard to pick a plan and keep me running — it takes a minute.", type: 'trial_expired', confidence: 1, suggestions: [], orderCard: null, messageId };
  }
  await db.addMessage(sessionId, 'user', message);
  // Sentiment tracking: rolling average per session drives ticket priority.
  const msgSentiment = sentiment.scoreMessage(message);
  if (msgSentiment !== 0) {
    const rolled = sentiment.rollAverage(session.sentiment || 0, msgSentiment);
    await db.updateSession(sessionId, { sentiment: rolled });
    session.sentiment = rolled;
  }
  if (isNewSession) {
    emit( 'conversation.started',
      { session_id: sessionId, channel: session.channel || 'web', visitor_label: visitorLabel });
  }
  emit( 'message.received',
    { session_id: sessionId, text: message.slice(0, 500), channel: session.channel || 'web' });
  ...
  // Live-agent takeover: a human is handling this conversation — the bot
  // stays silent (no LLM cost, no competing reply). The visitor's message is
  // already stored above; the agent picks it up via unread_admin.
  if (session.human_active) {
    await db.updateSession(sessionId, { unread_admin: 1 });
    return { sessionId, reply: '', type: 'human_active', confidence: 1, suggestions: [], orderCard: null, messageId: null };
  }
  ...
  const maybeDelegateAction = async (name, args) => {
    ...
    try {
      ...
      await audit(name, args, 'delegated');
      return true;
    } catch (err) {
      console.error('[bot] action webhook failed:', err.message);
      await audit(name, args, 'failed');
      return 'error';
    }
  };
  ...
  const runAction = async (action) => {
    /** Audit-log an action AND emit the action.executed webhook event. */
    const audit = async (actionName, actionArgs, status) => {
      await db.logAction(business.id, { session_id: sessionId, action: actionName, args: actionArgs, status });
      emit( 'action.executed',
        { action: actionName, args: actionArgs, status, session_id: sessionId });
    };
    ...
    // Supervised autonomy: mutating actions never execute — they route to a
    // human for approval instead (single choke point for both intent paths).
    if (autonomyLevel(business) === 'supervised' && (name === 'cancel_order' || name === 'issue_refund')) {
      await audit(name, { ...args, order_number: orderNumber }, 'pending_approval');
      await flagForHuman(sessionId,
        `${name === 'cancel_order' ? 'Order cancellation' : 'Refund'} requested — needs approval (supervised autonomy)`,
        message, isTest ? null : business.id);
      return finish(`I've sent that to the team for approval — they'll review it shortly. Anything else I can help with meanwhile?`,
        'fallback', { confidence: 1 });
    }

    if (name === 'escalate_to_human') {
      const reason = String(args.reason || '').slice(0, 200);
      await flagForHuman(sessionId, reason || 'visitor asked for a human', message, isTest ? null : business.id);
      await db.updateSession(sessionId, {
        capture_state: { step: 'awaiting_name', kind: 'callback', note: `Action escalation: ${reason || 'visitor asked for a human'}` },
      });
      await audit(name, args, 'completed');
    ...
    if (name === 'get_order_status') {
      // Live Shopify lookup first when a shop is connected; local DB is the fallback.
      const shop = await shopify.getConnectedShop(business.id).catch(() => null);
      if (shop && orderNumber) {
        const live = await shopify.findShopifyOrderByName(shop.shopDomain, shop.accessToken, orderNumber);
        if (live) {
          const ff = await shopify.getShopifyFulfillmentSummary(shop.shopDomain, shop.accessToken, live.id);
          const status = live.cancelled_at ? 'cancelled'
            : (live.fulfillment_status || live.financial_status || 'pending');
          const items = (live.line_items || []).map((li) => `${li.quantity}x ${li.title}`).join(', ').slice(0, 300);
          const card = {
            order_number: String(live.order_number || live.name).replace(/^#/, ''),
            status, eta: '', carrier: (ff && ff.carrier) || '',
            tracking_number: (ff && ff.tracking_number) || '', items,
          };
          await audit(name, { order_number: orderNumber, live: true }, 'completed');
          return finish(
            `Here's the latest on order ${card.order_number}: **${status}**${ff && ff.carrier ? ` via ${ff.carrier}` : ''}${ff && ff.tracking_number ? ` (tracking: ${ff.tracking_number})` : ''}${ff && ff.tracking_url ? `\nTrack it here: ${ff.tracking_url}` : ''}. Is there anything else I can help with?`,
      ...
      if (order) {
        ...
        await audit(name, { order_number: orderNumber }, 'completed');
        return finish(
      ...
      await audit(name, { order_number: orderNumber }, 'failed');
      return finish(
    ...
    if (name === 'cancel_order') {
      ...
      // Live cancel against Shopify when a shop is connected; local DB stays as fallback/audit.
      const shop = await shopify.getConnectedShop(business.id).catch(() => null);
      if (shop) {
        const live = await shopify.findShopifyOrderByName(shop.shopDomain, shop.accessToken, orderNumber);
        if (live && !live.cancelled_at) {
          const res = await shopify.cancelShopifyOrder(shop.shopDomain, shop.accessToken, live.id);
          if (!res.ok) {
            await audit(name, { order_number: orderNumber }, 'failed');
            await flagForHuman(sessionId, `Shopify cancel failed for order ${orderNumber}: ${res.error}`, message, isTest ? null : business.id);
            return finish(`I couldn't cancel order ${orderNumber} in the store just now (${res.error || 'store error'}) — I've flagged it for the team and they'll complete the cancellation shortly.`,
              'fallback', { confidence: 1 });
          }
        }
      }
      await db.updateOrderStatus(business.id, orderNumber, 'cancelled');
      await audit(name, { order_number: orderNumber, live: !!shop }, 'completed');
      return finish(`Done — order ${orderNumber} has been cancelled${shop ? ' in the store' : ''}. Anything else I can help with?`,
        'fallback', { confidence: 1 });
    }
    ...
    if (name === 'issue_refund') {
      ...
      const baseLimit = Number(getSetting(business, 'refund_auto_approve_limit', 50));
      const limit = autonomyLevel(business) === 'autonomous' ? baseLimit * 2 : baseLimit;
      const money = `$${amount.toFixed(2)}`;
      if (amount <= limit) {
        // Live refund against Shopify when a shop is connected; local DB stays as fallback/audit.
        const shop = await shopify.getConnectedShop(business.id).catch(() => null);
        let shopRes = null;
        if (shop) {
          const live = await shopify.findShopifyOrderByName(shop.shopDomain, shop.accessToken, orderNumber);
          if (!live) {
            await audit(name, { order_number: orderNumber, amount }, 'failed');
            return finish(`I couldn't find order ${orderNumber} in the store to refund — could you double-check the number?`,
              'fallback', { confidence: 0 });
          }
          shopRes = await shopify.refundShopifyOrder(shop.shopDomain, shop.accessToken, live, amount);
          const res = shopRes;
          if (!res.ok) {
            await audit(name, { order_number: orderNumber, amount }, 'failed');
            await flagForHuman(sessionId, `Shopify refund failed for order ${orderNumber} (${money}): ${res.error}`, message, isTest ? null : business.id);
            return finish(`I couldn't issue that refund in the store just now (${res.error || 'store error'}) — I've flagged it for the team and they'll complete it shortly.`,
              'fallback', { confidence: 1 });
          }
        }
        await db.updateOrderStatus(business.id, orderNumber, 'refunded');
        await audit(name, { order_number: orderNumber, amount, live: !!shop, partial: !!(shopRes && shopRes.partial) }, 'completed');
        const doneNote = shopRes && shopRes.partial ? `a partial refund of ${money}` : `a refund of ${money}`;
        return finish(`Done — ${doneNote} for order ${orderNumber} has been issued${shop ? ' in the store' : ''}. It should appear on your statement in a few business days.`,
          'fallback', { confidence: 1 });
      }
      await flagForHuman(sessionId, `refund approval needed: ${money} for order ${orderNumber}`, message, isTest ? null : business.id);
      await audit(name, { order_number: orderNumber, amount }, 'pending_approval');
  ...
  const handleToolCalls = async (toolCalls) => {
    for (const tc of toolCalls) {
      ...
      if (tc.name === 'cancel_order' || tc.name === 'issue_refund') {
        ...
        const args = { ...(tc.args || {}), order_number: normalizeOrderNumber(tc.args && tc.args.order_number) };
        if (autonomyLevel(business) === 'autonomous') return runAction({ name: tc.name, args }); // no confirmation
        await db.updateSession(sessionId, { capture_state: { step: 'awaiting_action_confirm', action: { name: tc.name, args } } });
        return finish(confirmPrompt(tc.name, args), 'capture', { confidence: 1 });
      }
  ...
  if (capture && capture.step) {
    ...
    // Agentic action follow-up: collect a missing order number / refund amount.
    if (capture.step === 'awaiting_action_details') {
      await db.updateSession(session.id, { capture_state: null });
      if (CANCEL_RE.test(message)) {
        return finish(`No problem — how else can I help?`, 'fallback',
          { confidence: 1, suggestions: await defaultSuggestions(business) });
      }
      const args = { ...(capture.action.args || {}) };
      if (!args.order_number) {
        const on = normalizeOrderNumber(message);
        if (on) args.order_number = on;
      }
      if (capture.action.name === 'issue_refund' && args.amount === undefined) {
        const am = message.match(AMOUNT_RE);
        if (am) args.amount = parseFloat(am[1] || am[2]);
      }
      const needOrder = !args.order_number;
      const needAmount = capture.action.name === 'issue_refund' && args.amount === undefined;
      if (needOrder || needAmount) {
        await db.updateSession(session.id, { capture_state: { step: 'awaiting_action_details', action: { name: capture.action.name, args } } });
        const need = needOrder ? 'order number (it looks like ORD-1234)' : 'refund amount in dollars';
        return finish(`I still need the ${need} to proceed — or say "cancel" to stop.`, 'capture', { confidence: 1 });
      }
      await db.updateSession(session.id, { capture_state: { step: 'awaiting_action_confirm', action: { name: capture.action.name, args } } });
      return finish(confirmPrompt(capture.action.name, args), 'capture', { confidence: 1 });
    }
  ...
  // ---- 1b. Deterministic agentic intents (cancel/refund) — no LLM required ----
  // Runs before order tracking so "cancel my order ORD-1002" reaches the
  // confirmation flow instead of being answered as a status lookup.
  const intent = actionIntent(message);
  if (intent && getSetting(business, 'actions_enabled', true)) {
    if (!actionEnabled(business, intent.name)) return runAction({ name: intent.name, args: {} });
    const args = {};
    if (intent.orderNumber) args.order_number = intent.orderNumber;
    if (intent.name === 'issue_refund' && Number.isFinite(intent.amount)) args.amount = intent.amount;
    const needOrder = !args.order_number;
    const needAmount = intent.name === 'issue_refund' && args.amount === undefined;
    if (needOrder || needAmount) {
      await db.updateSession(sessionId, { capture_state: { step: 'awaiting_action_details', action: { name: intent.name, args } } });
      const need = needOrder ? 'order number (it looks like ORD-1234)' : 'refund amount in dollars';
      return finish(`Sure — what's the ${need}?`, 'capture', { confidence: 1 });
    }
    if (autonomyLevel(business) === 'autonomous') return runAction({ name: intent.name, args });
    await db.updateSession(sessionId, { capture_state: { step: 'awaiting_action_confirm', action: { name: intent.name, args } } });
    return finish(confirmPrompt(intent.name, args), 'capture', { confidence: 1 });
  }
  ...
  if (ESCALATION_RE.test(message)) {
    await flagForHuman(sessionId, 'visitor asked for a human', message, isTest ? null : business.id);
  ...
  if (llmCfg.enabled) {
    try {
      ...
      let answer;
      if (actionsOn) {
        const t = await llm.chatCompleteToolsMetered(business, llmCfg, messages, AGENT_TOOLS);
        ...
        answer = await llm.chatCompleteMetered(business, llmCfg, messages);
      }
...
async function processCapture(business, session, capture, message) {
  ...
  if (capture.step === 'awaiting_phone') {
    ...
    emit( 'lead.captured', {
      session_id: session.id, name: data.name, email: data.email,
      phone: data.phone, kind: capture.kind || 'lead',
    });
