/**
 * Chat engine: intent detection -> FAQ retrieval -> LLM fallback -> lead capture.
...
const ANSWER_THRESHOLD = 0.35;  // FAQ cosine score: answer directly at/above this
...
function chipLabels(business) {
  const labels = ['track my order', 'track another order', 'talk to a human', 'yes, thanks', 'return policy'];
  try {
    for (const f of db.listFaqs(business.id)) labels.push(String(f.question).toLowerCase());
  } catch {}
  return labels;
}

// True when the message looks like a command/intent rather than a person's name.
function looksLikeCommand(business, message) {
  if (chipLabels(business).includes(message.toLowerCase())) return true;
 ...
 * @returns {Promise<{sessionId, reply, type, confidence, suggestions, orderCard}>}
 */
async function processMessage({ business, apiKey, sessionId, text, visitorLabel = '' }) {
  let session = sessionId && db.getSession(sessionId);
  if (!session || session.business_id !== business.id) {
    session = db.createSession(business.id, visitorLabel);
  }
  sessionId = session.id;
  const message = (text || '').trim();
  db.addMessage(sessionId, 'user', message);

  const finish = (reply, type, { confidence = 0, faq_id = null, suggestions = [], orderCard = null } = {}) => {
    db.addMessage(sessionId, 'assistant', reply,
      { type, confidence, faq_id, business: business.name }, orderCard ? 'order_card' : 'text');
    return { sessionId, reply, type, confidence, suggestions, orderCard };
  };

  if (!message) {
    return finish(getSetting(business, 'welcome_message', `Hi! How can I help you today?`), 'fallback',
      { confidence: 0, suggestions: defaultSuggestions(business) });
  }
  ...
  if (capture && capture.step) {
    ...
    if (CANCEL_RE.test(message)) {
      db.updateSession(session.id, { capture_state: null });
      return finish(`No problem — how else can I help?`, 'fallback',
        { confidence: 1, suggestions: defaultSuggestions(business) });
    }
    ...
    if (capture.step === 'awaiting_name' && looksLikeCommand(business, message)) {
      db.updateSession(session.id, { capture_state: null });
      capture = null;
    }
  }
  ...
  if (orderMatch) {
    ...
    const order = db.findOrder(business.id, orderNumber);
    if (order) {
  ...
  if (GREETING_RE.test(message) && message.length < 30) {
    return finish(getSetting(business, 'welcome_message', `Hi! I'm the ${business.name} assistant. How can I help you today?`),
      'smalltalk', { confidence: 1, suggestions: defaultSuggestions(business) });
  }
  ...
  // ---- 6. FAQ retrieval ----
  const faqs = db.listFaqs(business.id);
  const retriever = buildRetriever(faqs);
  const hits = retriever.search(message, 3);
  const best = hits[0];
  ...
  if (llmCfg.enabled) {
    try {
      const history = db.getHistory(sessionId, 10);
      const messages = llm.buildKbMessages({
        businessName: business.name,
        faqs: hits.map(h => h.faq),
  ...
  if (best && best.score >= LOW_THRESHOLD) {
    return finish(
      `I'm not 100% sure, but this might help:\n\n**${best.faq.question}**\n${best.faq.answer}\n\nDid that answer your question? If not, I can have someone from our team reach out.`,
      'fallback', { confidence: +best.score.toFixed(3), faq_id: best.faq.id, suggestions: ['Yes, thanks', 'Talk to a human'] });
  }
  ...
  return finish(
    ...
    'fallback', { confidence: 0, suggestions: defaultSuggestions(business) });
}
...
async function processCapture(business, session, capture, message) {
  ...
  const done = (reply) => {
    db.updateSession(session.id, { capture_state: null });
    return [reply, 'capture', { suggestions: defaultSuggestions(business) }];
  };
  ...
  if (capture.step === 'awaiting_name') {
    ...
    data.name = message;
    db.updateSession(session.id, { capture_state: { ...capture, step: 'awaiting_email', data } });
  ...
  if (capture.step === 'awaiting_email') {
    if (/^(skip|no|n\/a|none)$/i.test(message)) {
      data.email = '';
      db.updateSession(session.id, { capture_state: { ...capture, step: 'awaiting_phone', data } });
  ...
  if (capture.step === 'awaiting_phone') {
    ...
    const lead = db.addLead(business.id, {
...
function defaultSuggestions(business) {
  const faqs = db.listFaqs(business.id).slice(0, 3).map(f => f.question);
