    ...
    if (name === 'issue_refund') {
      ...
      if (amount <= limit) {
        ...
        if (shop) {
          ...
          const res = await shopify.refundShopifyOrder(shop.shopDomain, shop.accessToken, live);
          if (!res.ok) {
            await audit(name, { order_number: orderNumber, amount }, 'failed');
        ...
        await audit(name, { order_number: orderNumber, amount, live: !!shop }, 'completed');
        return finish(`Done — a refund of ${money} for order ${orderNumber} has been issued${shop ? ' in the store' : ''}. It should appear on your statement in a few business days.`,
          'fallback', { confidence: 1 });
      }
