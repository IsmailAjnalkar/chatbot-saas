...
async function flagForHuman(sessionId, reason, lastVisitorMessage, businessId) {
  ...
  if (businessId) {
    ...
    try {
      ...
      if (!existing) {
        const ticket = await db.createTicket(businessId, { session_id: sessionId, subject: r });
        webhooks.emit(businessId, 'ticket.created', { ticket }).catch(() => {});
      }
    } catch (e) { console.warn('[bot] ticket create failed:', e.message); }
    webhooks.emit(businessId, 'handoff.requested', { session_id: sessionId, reason: r }).catch(() => {});
  }
}
...
async function processMessage({ business, apiKey, sessionId, text, visitorLabel = '' }) {
  ...
  if (isNewSession) {
    webhooks.emit(business.id, 'conversation.started',
      { session_id: sessionId, channel: session.channel || 'web', visitor_label: visitorLabel }).catch(() => {});
  }
  webhooks.emit(business.id, 'message.received',
    { session_id: sessionId, text: message.slice(0, 500), channel: session.channel || 'web' }).catch(() => {});
  ...
  const runAction = async (action) => {
    ...
    const audit = async (actionName, actionArgs, status) => {
      ...
      webhooks.emit(business.id, 'action.executed',
        { action: actionName, args: actionArgs, status, session_id: sessionId }).catch(() => {});
    };
    ...
    if (name === 'escalate_to_human') {
      ...
      await flagForHuman(sessionId, reason || 'visitor asked for a human', message, business.id);
    ...
    if (name === 'cancel_order') {
      ...
      if (shop) {
        ...
        if (live && !live.cancelled_at) {
          ...
          if (!res.ok) {
            await audit(name, { order_number: orderNumber }, 'failed');
            await flagForHuman(sessionId, `Shopify cancel failed for order ${orderNumber}: ${res.error}`, message, business.id);
    ...
    if (name === 'issue_refund') {
      ...
      if (amount <= limit) {
        ...
        if (shop) {
          ...
          if (!res.ok) {
            await audit(name, { order_number: orderNumber, amount }, 'failed');
            await flagForHuman(sessionId, `Shopify refund failed for order ${orderNumber} (${money}): ${res.error}`, message, business.id);
      ...
      await flagForHuman(sessionId, `refund approval needed: ${money} for order ${orderNumber}`, message, business.id);
  ...
  if (ESCALATION_RE.test(message)) {
    await flagForHuman(sessionId, 'visitor asked for a human', message, business.id);
...
async function processCapture(business, session, capture, message) {
  ...
  if (capture.step === 'awaiting_phone') {
    ...
    webhooks.emit(business.id, 'lead.captured', {
      session_id: session.id, name: data.name, email: data.email,
