 ...
 */
async function flagForHuman(sessionId, reason, lastVisitorMessage) {
...
async function processMessage({ business, apiKey, sessionId, text, visitorLabel = '' }) {
  ...
  const runAction = async (action) => {
    ...
    if (name === 'escalate_to_human') {
      ...
      await flagForHuman(sessionId, reason || 'visitor asked for a human', message);
    ...
    if (name === 'issue_refund') {
      ...
      await flagForHuman(sessionId, `refund approval needed: ${money} for order ${orderNumber}`, message);
  ...
  if (ESCALATION_RE.test(message)) {
    await flagForHuman(sessionId, 'visitor asked for a human', message);
