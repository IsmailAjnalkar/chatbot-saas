...
async function handleChannelMessage(business, channel, sender, text) {
  ...
  // During a live-agent takeover the bot stays silent — the agent replies
  // from the inbox. (processMessage records the visitor message and marks
  // it unread for the agent.)
  if (result.type === 'human_active' || !result.reply) return result;
  try {
